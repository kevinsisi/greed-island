...
✅ **ARCHITECTURE.md §11.5 FACT_SET** — All major runtime domains now have typed EventLog projections: NPC state (NpcStateProjection), area state (AreaStateProjection), weather/season/rare window/active events (WorldStateProjection v0.72.0), building occupants (BuildingOccupantsProjection v0.73.0). FACT_SET is still emitted on some live world-state paths (area/weather/season/rare-window) and still serves as a fallback during availability-first large-log boot, so the migration is substantial but not fully retired.
...
**Household economy** (4)
`HOUSEHOLD_GOLD_CONTRIBUTED`, `HOUSEHOLD_GOLD_SPENT`, `HOUSEHOLD_INHERITANCE_ASSIGNED`（v0.88.0 起由 mortality cadence 實際發出，並攜帶 `goods` 清單把死者名下庫存搬給繼承人）, `NPC_INHERITANCE_GRANTED`（v0.88.0 — 成年孩子從父母 civic 紀錄均值換算的起步 seed；非轉移）
...
✅ §6.5 event catalog now complete (v0.55.0–v0.61.0): `HIDE_COLLECTED` (v0.61.0), `BONE_COLLECTED` (v0.61.0), `BIOME_RECOVERED` (v0.56.0), `FOREST_DEPLETED` (v0.55.0), `POLLUTION_INCREASED` (v0.60.0), `POLLUTION_RECOVERED` (v0.60.0), `SPECIES_POPULATION_SHIFTED` (v0.59.0) all shipped.
...
✅ **ARCHITECTURE.md §11.2 — Cards as World Rule Operators (Phase 4, v0.76.0):**
- `CardRuleOperatorDef` type on `CardCatalogEntry` (optional); 3 catalog entries (IDs 11, 31, 51) ship with `ruleOperator` definitions
- `ActiveRuleOperatorsProjection` tracks `CARD_RULE_OPERATOR_ACTIVATED` / `CARD_RULE_OPERATOR_EXPIRED` events; `getPriceMultiplier()` + `getProductionMultiplier()` for O(n) multiplier lookup
- `PLAYER_PLAYED_CARD` → auto-emits `CARD_RULE_OPERATOR_ACTIVATED` in `publishCommittedEvents`; operator expires in runTick cadence
- Price multipliers injected into `discoverMarketPrices()`; production multipliers applied per recipe output in `GOODS_PROCESSED` command generation
- Cards are now genuine world rule operators — playing a card modifies simulation rule evaluation for a bounded duration
