...
describe('SimulationRuntime life goals and expansion', () => {
  it('turns NPC life pressure and productive work into committed expansion facts', () => {
    ...
    try {
      ...
      expect(productiveData?.motivation?.explanation).toContain('公共進展')
      expect(productiveData?.motivation?.explanation ?? '').not.toContain('agenda.')
      expect(productiveData?.motivation?.projectPurpose ?? '').not.toContain('agenda.')
      expect(lifeGoalData?.motivation?.explanation).toContain('生活壓力')
      expect(lifeGoalData?.motivation?.explanation).toContain('需求')
      expect(lifeGoalData?.motivation?.explanation ?? '').not.toContain('agenda.')
      expect(lifeGoalData?.motivation?.projectPurpose ?? '').not.toContain('agenda.')
