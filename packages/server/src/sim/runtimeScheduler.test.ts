...
describe('SimulationRuntime tick scheduler', () => {
  ...
  it('adds an HTTP recovery window after slow ticks', () => {
    expect(computeNextTickDelayMs({ tickDurationMs: 5_000, elapsedMs: 13_000 })).toBe(30_000)
    expect(computeNextTickDelayMs({ tickDurationMs: 5_000, elapsedMs: 24_000 })).toBe(48_000)
  })

  it('caps slow-tick cooldown so simulation does not stop progressing', () => {
