...
describe('SimulationRuntime tick scheduler', () => {
  ...
  it('bounds one scheduler callback while preserving catch-up pressure for later turns', () => {
    expect(computeDueTickCount({ tickDurationMs: 5_000, nowMs: 65_000, nextDueAtMs: 5_000, maxCatchUpTicks: 3 })).toBe(3)
  })
})
