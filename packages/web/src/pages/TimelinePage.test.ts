...
describe('Timeline event motivation', () => {
  ...
  it('explains common non-construction public events', () => {
    ...
    expect(eventMotivationFor(event('NPC_FREEFORM_ACTION_PROPOSED', { accepted: true, resolved: { kind: 'buy_goods' } }))?.explanation).toContain('購物 / 採買')
    expect(eventMotivationFor(event('NPC_FREEFORM_ACTION_PROPOSED', { accepted: true, resolved: { kind: 'learn' } }))?.explanation).toContain('學習 / 拜師')
    expect(eventMotivationFor(event('NPC_FREEFORM_ACTION_PROPOSED', { accepted: true, resolved: { kind: 'invent' } }))?.explanation).toContain('發想 / 原型')
