# ecosystem-aggression Specification

## Purpose

First slice of the `ecosystem-aggression` capability. Closes the "ecosystem cannot bite" gap noted in `docs/WORLD_CAPABILITIES.md` Part I §6.2 — starving predators gain agency over NPCs on their tile, and hunted prey can land a retaliation blow before dying. All aggression interactions flow through the same Command → Rule Engine → Event pipeline as the rest of the living world, so replay determinism is preserved.
