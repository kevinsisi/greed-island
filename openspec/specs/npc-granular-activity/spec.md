# npc-granular-activity Specification

## Purpose
Extends the NpcActivity type with role-specific fine-grained values so that NPC schedule
labels are accurately reflected in both server state and client animations. Replaces the
coarse `work` fallback for librarians, performers, crafters, scholars, priests, and scribes
with distinct activity types that drive unique character animations and emoji glyphs.
...
Patterns are checked in order before LABEL_WORK_PATTERN so specific roles are not collapsed to `work`.
