# npc-schedule-building-placement Specification

## Purpose
Adds an optional `buildingId` field to NPC schedule slots so NPCs are placed inside
the correct building during that slot. Ensures role-assigned NPCs (librarians, priests,
blacksmiths) actually appear inside their designated building during work hours, and
that AI dialog context correctly reflects the NPC's current building.
