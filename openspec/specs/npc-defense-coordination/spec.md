# npc-defense-coordination Specification

## Purpose

Defines the civilization-side response to ecological aggression. When an
animal attacks an NPC and other NPCs are nearby on the same tile, the
neighbours coordinate a defense party and put the predator down. This
closes the loop between Part I §6.2 (ecological pressure) and §5.1 (NPC
humanity) of the world capabilities constitution — animals do not get to
freely kill villagers while bystanders watch. The capability runs
through the same Command → Rule Engine → Event pipeline as every other
living-world state change, so defense reactions are deterministic and
replayable.
