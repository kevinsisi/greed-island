...
## Purpose
Defines the lifecycle that promotes a born child (recorded via `NPC_CHILD_BORN` in `LifeExpansionState.households[].childIds`) into a fully-formed runtime NPC entity capable of holding beliefs, intentions, memory, dialog, and presence. Closes the gap whereby human births fire events without producing runtime entities, breaking the §43.1 first acceptance criterion ("when an NPC dies, descendants remember them").
