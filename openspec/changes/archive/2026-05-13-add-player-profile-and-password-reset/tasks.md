...
- [x] Update memory `project_deploy_state.md` to reflect v0.6.0.
- [x] `docker compose -f deploy/docker-compose.yml --env-file deploy/.env up -d --build` on the desktop and verify `/api/version` returns 0.6.0, `/profile` saves nickname/avatar, `/forgot-password` mints a token, and `/reset-password?token=<token>` rotates the password.
