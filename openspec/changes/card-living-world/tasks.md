...
- [ ] 5.3 部署到本機 docker compose 後 `curl https://hunter.sisihome.org/healthz` 應回 `version:"0.13.0"`
- [ ] 5.4 hub 進場應看到 since-last-visit toast（如有 spawn 累積）
- [ ] 5.5 區域內地圖看到 drop spawn 在鏽灣偏 SS/S/A、潮聲偏 G/H；雨天比晴天密
- [ ] 5.6 work N 個 shift 把 energy < 30，再撿一張卡，觀察 holdingTimer 顯示與真實 60s 偏差 ±5s
- [ ] 5.7 Codex 點實體化按鈕需要長按 2 秒才送 request；放開取消
- [ ] 5.8 hub 城市地圖看到 NPC sprite 在 district 內緩慢移動（不再釘 anchor）；跨 district 看到 4.5s tween 滑過去
