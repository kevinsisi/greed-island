...
## Purpose
Replaces the single 0..100 `trust` scalar on each NPC pair with an eight-axis directional vector `{ trust, fear, respect, attraction, loyalty, resentment, dependency, familiarity }` per WORLD_CAPABILITIES.md §12.5.12. Stores per ordered `(from, to)` direction so asymmetric feelings (mentor↔apprentice, aggressor↔victim) are first-class.
